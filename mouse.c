...
void mouse_write(uchar cmd)
{
	outb(0x64, 0xd4);
	outb(0x60, cmd);
}

void mouse_awrite(uchar cmd, uchar addtional_cmd)
{
	outb(0x64, addtional_cmd);
	outb(0x60, cmd);
}

uchar mouse_read()
{
	uchar result = inb(0x60);
	if (result == 0xFE)
	{
		cprintf("Mouse read error!");
	}
	return result;
}
...
void mouseInterupt(int ticks)
{
	...
	case 2:
	{
		int x_offset = (x_sign | ch);
		//int x_offset = (int)((float)(x_sign | ch) / 10);
		//cprintf("X_Offset:%d, ", x_offset);
		x_position += x_offset;
		if (x_position < 0)
		{
			x_position = 0;
		...
		{
			x_position = videoWidth - 2;
		}
	}
	 	break;
	case 3:
	{
		int y_offset = -(y_sign | ch);
		//int y_offset = -(int)((float)(y_sign | ch) / 10);
		//cprintf("Y_Offset:%d\n", y_offset);
		y_position += y_offset;
		if (y_position < 0)
		{
	...
	case 4:
	{
		cprintf("HAHAHAA I Got the 4th byte!!!");
	}
	break;
  default:
	  break;
